...
| ロール | 想定する人 | 権限 |
|---|---|---|
| **管理者 (Admin)** | 総務部門の責任者 | 全機能。マスタ編集、ユーザー管理、監査ログ閲覧 |
| **在庫担当 (Staff)** | 各部署の備品係 | 入出庫・貸出返却の登録、棚卸しの実施、備品の登録・編集 |
| **一般 (Member)** | 全社員 | 在庫の閲覧、自分の貸出状況の確認、貸出申請 |
...
| ID | 機能 | 優先度 | 概要 |
|---|---|---|---|
| F-101 | ログイン / ログアウト | Must | メールアドレス＋パスワード |
| F-102 | ユーザー一覧・登録・編集・無効化 | Must | 管理者のみ。削除は論理削除 |
| F-103 | ロール割当 | Must | Admin / Staff / Member |
| F-104 | パスワード変更 | Must | 本人のみ |
| F-105 | パスワードリセット（メール） | Should | |
| F-106 | 部署マスタと所属管理 | Could | 部署別の使用量集計に使う |
...
- ディレクトリは機能単位（`features/items`, `features/loans` …）で切る
...
| フェーズ | 内容 | 完了条件 |
|---|---|---|
| M0 | 環境構築、DB 設計、認証 | Docker で起動しログインできる |
